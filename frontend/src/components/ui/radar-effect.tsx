...
export const Circle = ({ className, children, idx, ...rest }: any) => {
  return (
    <motion.div
      ...
      className={twMerge(
        "absolute inset-0 left-1/2 top-1/2 h-10 w-10 -translate-x-1/2 -translate-y-1/2 transform rounded-full border border-sky-500/30",
        className
      )}
    />
  );
...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-transparent via-sky-600 to-transparent shadow-[0_0_10px_2px_rgba(2,132,199,0.5)]" />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(14, 165, 233, ${1 - (idx + 1) * 0.1})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      ...
      <div className="hidden rounded-md px-2 py-1 md:block bg-black/50 border border-slate-800">
        <div className="text-center text-xs font-bold text-sky-400 font-mono tracking-widest">
          {text || "SCANNING"}
        </div>
      </div>
    </motion.div>
